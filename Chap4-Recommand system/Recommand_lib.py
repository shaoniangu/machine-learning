...
def file2matrix(path, delimiter):
    ...
    # 逐行遍历 		# 结果按分隔符分割为行向量
    recordlist=[map(eval, row.decode().split(delimiter)) for row in rowlist if row.strip() ]
...
# path: 数据文件路径
# delimiter: 文件分隔符
def file2matrix(path, deli):
    A = np.loadtxt(open(path),delimiter=deli,skiprows=0)
    return A
...
# 绘制散点图
def drawScatter_origin(plt,mydata,size=20,color='blue',mrkr='o'):
    print(mydata)
    print(mydata.T[0].tolist())
    print(mydata.T[1].tolist())
    #print(mydata.T[0].tolist()[0])
    #print(mydata.T[1].tolist()[0])
    plt.scatter(mydata.T[0].tolist()[0],mydata.T[1].tolist()[0],s=size,c=color,marker=mrkr)

def drawScatter_center(plt, mydata, size=20, color='blue', mrkr='o'):
    print(mydata)
    print(mydata.T[0].tolist())
    print(mydata.T[1].tolist())
